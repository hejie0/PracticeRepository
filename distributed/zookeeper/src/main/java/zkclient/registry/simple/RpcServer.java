...
import zkclient.registry.register.RegistCenter;
import zkclient.registry.register.ServiceResource;
...
public class RpcServer {
	...
	/**
	 * 启动指定的网络端口号服务，并监听端口上的请求数据。
	 * 获得请求数据以后将请求信息委派给服务处理器，放入线程池中执行。
	 * @param port 监听端口
	 * @param clazz 服务类所在包名，多个用英文逗号隔开，类似spring的包扫描
	 */
	...
	private void rigester(Map<String, Object> services, int port) {
		RegistCenter center = new RegistCenter();
		// 循环实例化
		services.forEach((k, v)->{
			ServiceResource serviceResource = new ServiceResource();
			String serviceName = k;
			serviceResource.setHost("127.0.0.1");
			serviceResource.setPort(port);
			...
			serviceResource.setMethods(methods);
			serviceResource.setServiceName(serviceName);
			center.regist(serviceResource);
		});
	}
